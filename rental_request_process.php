...
// Store POST data in session so back button works
...
// ── Fetch all payment methods for card selection ──────────────────────────────
$allCards    = [];
$primaryCard = null;
try {
    ...
    $allCards = $cardStmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($allCards as $c) {
        if ($c['PrimaryCard']) { $primaryCard = $c; break; }
    }
    if (!$primaryCard && !empty($allCards)) $primaryCard = $allCards[0];
} catch (PDOException $e) {
    $allCards = [];
}

// Determine selected card (from POST if re-displaying, else primary)
$selectedCardId = (int)($_POST['selected_card_id'] ?? ($primaryCard['CardID'] ?? 0));

// If no card on file, block with message rather than silent redirect
if (empty($allCards) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $_SESSION['confirm_data'] = $_SESSION['confirm_data'] ?? [];
    // Store session so we can come back
    session_write_close();
    header('Location: account_info.php?no_card=1&return_to=' . urlencode('rental_request_process.php') . '#payment-methods');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_submit'])) {
    // Block if no payment method
    if (empty($allCards)) {
        $error = 'You must add a payment method before requesting a rental. <a href="account_info.php?no_card=1&return_to=' . urlencode('rental_request_process.php') . '#payment-methods" style="color:#667eea;font-weight:700;">Add one now →</a>';
    } else {
    $selectedCardId = (int)($_POST['selected_card_id'] ?? ($primaryCard['CardID'] ?? 0));
    // Validate the selected card belongs to this user
    $validCard = false;
    foreach ($allCards as $c) {
        if ((int)$c['CardID'] === $selectedCardId) { $validCard = true; break; }
    }
    if (!$validCard) $selectedCardId = (int)($primaryCard['CardID'] ?? 0);
    ...
    try {
        ...
        if ($blockedStmt->fetchColumn() > 0) {
            ...
            if ($conflictStmt->fetchColumn() > 0) {
                ...
                // Insert rental request, store selected card in session for accept_rental
                $insertStmt = $pdo->prepare("
                    INSERT INTO TRentalRequests (ListingID, UserBorrowerID, StartDate, EndDate, RequestStatusID, RequestDate)
                    VALUES (?, ?, ?, ?, 1, NOW())
                ");
                $insertStmt->execute([$listingId, $userId, $startStr, $endStr]);
                $newRequestId = $pdo->lastInsertId();

                // Store borrower's chosen card so accept_rental can use it
                $_SESSION['borrower_card_' . $newRequestId] = $selectedCardId;

                // Clear session confirm data
                unset($_SESSION['confirm_data']);

                header('Location: my_rentals.php?tab=pending&requested=1');
                exit;
            }
        }
    } catch (PDOException $e) {
...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .confirm-actions { flex-direction: column-reverse; gap: 12px; padding: 18px; }
...
<body>
    ...
    <main class="main-content">
        <div class="confirm-wrap">
            ...
            <p class="page-sub">Review your rental details before sending the request to the owner.</p>
                ...
                <div>
                    ...
                    <div class="confirm-section">
                        ...
                        <?php if (!empty($allCards)): ?>
                            <p style="font-size:13px;color:#6b7280;margin-bottom:12px;">Select which card to charge for this rental. Your primary card is selected by default.</p>
                            <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:14px;">
                                <?php foreach ($allCards as $card):
                                    $isSelected = (int)$card['CardID'] === $selectedCardId;
                                    $cardIconMap = ['Visa'=>'fa-cc-visa','Mastercard'=>'fa-cc-mastercard','American Express'=>'fa-cc-amex','Discover'=>'fa-cc-discover'];
                                    $cardIcon = $cardIconMap[$card['CardTypeName']] ?? 'fa-credit-card';
                                ?>
                                    <label style="display:flex;align-items:center;gap:12px;padding:12px 16px;border:<?php echo $isSelected ? '2px solid #667eea;background:#f5f3ff' : '1.5px solid #e5e7eb;background:white'; ?>;border-radius:10px;cursor:pointer;transition:border-color 0.15s;">
                                        <input type="radio" name="selected_card_id" value="<?php echo $card['CardID']; ?>"
                                               <?php echo $isSelected ? 'checked' : ''; ?>
                                               style="accent-color:#667eea;width:16px;height:16px;flex-shrink:0;"
                                               onchange="this.closest('.confirm-section').querySelectorAll('label').forEach(l=>l.style.border='1.5px solid #e5e7eb');this.parentElement.style.border='2px solid #667eea';">
                                        <i class="fab <?php echo $cardIcon; ?>" style="font-size:24px;color:#667eea;flex-shrink:0;"></i>
                                        <div style="flex:1;">
                            ...
                            <a href="account_info.php?return_to=<?php echo urlencode('rental_request_process.php'); ?>#payment-methods" style="font-size:12px;color:#667eea;font-weight:600;text-decoration:none;">
                                <i class="fas fa-plus-circle"></i> Add another card
                            </a>
                            <div style="margin-top:14px;background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:11px 14px;font-size:13px;color:#92400e;">
                                <i class="fas fa-info-circle"></i>
                                A <strong>10% deposit ($<?php echo number_format($totalCost * 0.10, 2); ?>)</strong> will be charged to your selected card when the rental begins.
                                The remaining <strong>$<?php echo number_format($totalCost * 0.90, 2); ?></strong> will be charged when the rental is returned.
                            </div>
                        <?php else: ?>
                            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:9px;padding:14px 16px;font-size:13px;color:#b91c1c;display:flex;align-items:center;gap:10px;">
                                <i class="fas fa-exclamation-circle" style="flex-shrink:0;"></i>
                                <span>You need a payment method to request a rental.
                                    <a href="account_info.php?no_card=1&return_to=<?php echo urlencode('rental_request_process.php'); ?>#payment-methods" style="color:#b91c1c;font-weight:700;text-decoration:underline;">Add one now →</a>
                                </span>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="confirm-actions">
                        ...
                        <form method="POST" action="rental_confirm.php" style="display:inline;">
                            ...
                            <input type="hidden" name="selected_card_id" value="<?php echo (int)$selectedCardId; ?>">
                            <button type="submit" class="btn-confirm" <?php echo empty($allCards) ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : ''; ?>>
