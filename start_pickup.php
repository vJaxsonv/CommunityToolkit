...
<div class="pu-card">
    <h3>Start Pickup</h3>
    <p>Add photos or videos to confirm pickup for <strong><?php echo htmlspecialchars($rental['Title']); ?></strong>.</p>

    <form id="pickupConfirmForm" enctype="multipart/form-data">
        ...
        <!-- Hidden file input -->
        <input type="file" id="pickupFileInput" name="pickup_media[]"
               accept="image/*,video/*" multiple style="display:none;">

        <!-- Drop zone -->
        <div class="pu-dropzone" id="pickupDropzone">
        ...
        <!-- Camera button -->
        <button type="button" class="pu-camera-btn" id="pickupCameraBtn">
            <i class="fas fa-camera"></i> Take a Photo
        </button>

        <!-- Preview grid -->
        <div class="pu-preview-wrap" id="pickupPreviewWrap" style="display:none;">
            <div class="pu-preview-grid" id="pickupPreviewGrid"></div>
        </div>

        <div class="pu-actions">
            <button type="submit" class="pickup-btn pickup-btn-primary">Confirm Pickup</button>
            <button type="button" class="pickup-btn pickup-btn-secondary"
                    onclick="document.getElementById('closePickupModal').click()">Cancel</button>
        </div>

        <div class="pu-status" id="pickupStatusMsg"></div>
    </form>
</div>

<!-- Live camera modal -->
<div class="pu-cam-modal" id="pickupCamModal">
    <video id="pickupCamVideo" autoplay playsinline muted></video>
    <div class="pu-cam-err" id="pickupCamErr"></div>
    <div class="pu-cam-btns">
        <button type="button" class="pu-cam-capture" id="pickupCamCapture">
            <i class="fas fa-camera"></i> Capture
        </button>
        <button type="button" class="pu-cam-close" id="pickupCamClose">Cancel</button>
    </div>
</div>
<canvas id="pickupCamCanvas" style="display:none;"></canvas>
...
(function () {
    ...
    const form        = document.getElementById('pickupConfirmForm');
    const fileInput   = document.getElementById('pickupFileInput');
    const dropzone    = document.getElementById('pickupDropzone');
    const previewWrap = document.getElementById('pickupPreviewWrap');
    const previewGrid = document.getElementById('pickupPreviewGrid');
    const statusMsg   = document.getElementById('pickupStatusMsg');
    const cameraBtn   = document.getElementById('pickupCameraBtn');
    const camModal    = document.getElementById('pickupCamModal');
    const camVideo    = document.getElementById('pickupCamVideo');
    const camCanvas   = document.getElementById('pickupCamCanvas');
    const camCapture  = document.getElementById('pickupCamCapture');
    const camClose    = document.getElementById('pickupCamClose');
    const camErr      = document.getElementById('pickupCamErr');
    ...
    camCapture.addEventListener('click', () => {
        ...
        camCanvas.toBlob(blob => {
            if (!blob) return;
            const file = new File([blob], 'pickup_' + Date.now() + '.jpg', { type: 'image/jpeg' });
            addFiles([file]);
        }, 'image/jpeg', 0.92);
    });
    ...
    form.addEventListener('submit', e => {
        ...
        // Ensure all files are in formData
        formData.delete('pickup_media[]');
        selectedFiles.forEach(f => formData.append('pickup_media[]', f));

        fetch('save_pickup_photo.php', { method: 'POST', body: formData })
            .then(r => r.json())
            .then(data => {
                statusMsg.className = 'pu-status ' + (data.success ? 'success' : 'error');
                statusMsg.textContent = data.message || (data.success ? 'Pickup saved.' : 'Something went wrong.');
                if (data.success) {
                    setTimeout(() => {
                        const modal = document.getElementById('pickupModal');
                        const content = document.getElementById('pickupModalContent');
                        if (modal) modal.classList.remove('active');
                        if (content) content.innerHTML = '<div class="pickup-loading">Loading...</div>';
                        location.reload();
                    }, 800);
                }
            })
            .catch(() => {
                statusMsg.className = 'pu-status error';
                statusMsg.textContent = 'Something went wrong saving the pickup.';
            });
    });
})();
</script>
