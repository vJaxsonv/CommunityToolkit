...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // If user has account location, don't bother with browser location
    if (hasAccountLocation) {
        return;
    }
    
    // If we already have location in localStorage, just refresh it silently
    if (LocationManager.hasStoredLocation()) {
    ...
    if (navigator.permissions) {
        try {
            ...
            if (permissionStatus.state === 'granted') {
                // Permission already granted, get location silently without prompt
                await LocationManager.requestBrowserLocation();
                return;
            }
        } catch (err) {
    ...
    // Show prompt only if no stored location and prompt not recently dismissed
    if (LocationManager.shouldShowPrompt()) {
